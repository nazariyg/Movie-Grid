...
public extension Reactive where Base: SessionManager {
    ...
    func making<ResponsePayload>(
        ...
        -> SignalProducer<HTTPResponse<ResponsePayload>, CoreError> {

        // Output injection.
        if let injectedOutputResult = InstanceProvider.shared.outputForInput(request, outputType: HTTPResponse<ResponsePayload>.self) {
            var signalProducer = SignalProducer<HTTPResponse<ResponsePayload>, CoreError>(value: injectedOutputResult.output)
        ...
        return
            SignalProducer { [weak base] emitter, lifetime in
                ...
                let requestDescription =
                    !requestHasSensitiveData ?
                        request.description(logParameters: Config.shared.general.logHTTPRequestParameters) :
                        request.safeDescription
                log.debug("Making an HTTP request: \(requestDescription)", logCategory)
    ...
    func downloading(url: URL, acceptForValidation: [HTTPContentType] = []) -> SignalProducer<DownloadStatus, CoreError> {
        return
            SignalProducer { [weak base] emitter, lifetime in
                guard let sessionManager = base else {
                    emitter.sendInterrupted()
    ...
    func uploadingFile(withURL fileURL: URL, toURL url: URL) -> SignalProducer<UploadStatus, CoreError> {
        return
            SignalProducer { [weak base] emitter, lifetime in
                guard let sessionManager = base else {
                    emitter.sendInterrupted()
    ...
    func uploadingData(_ data: @autoclosure @escaping () -> Data, toURL url: URL) -> SignalProducer<UploadStatus, CoreError> {
        return
            SignalProducer { [weak base] emitter, lifetime in
                guard let sessionManager = base else {
                    emitter.sendInterrupted()
    ...
    func makingForData(request: HTTPRequest, acceptForValidation: [HTTPContentType] = []) -> SignalProducer<HTTPDataResponse, CoreError> {
    ...
    private static func processURLResponseCodeForErrors<ResponsePayload>(
        _ urlResponse: HTTPURLResponse?, error: Error?, emitter: Signal<ResponsePayload, CoreError>.Observer) -> Bool {
