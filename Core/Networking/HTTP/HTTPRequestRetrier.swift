...
public struct HTTPRequestRetrier {
    ...
    public func shouldRetryRequest(withRetryCount retryCount: Int, error: CoreError, responseStatusCode: Int?) -> Bool {
...
extension HTTPRequestRetrier: Alamofire.RequestRetrier {
    ...
    public func should(_ manager: SessionManager, retry request: Request, with error: Error, completion: @escaping RequestRetryCompletion) {
        ...
        let shouldRetry = shouldRetryRequest(withRetryCount: Int(request.retryCount), error: CoreError(error), responseStatusCode: responseStatusCode)
        if shouldRetry {
